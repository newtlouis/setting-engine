...
async function migrate() {
  ...
  try {
    ...
    const hasBookingStatus = tableInfo.some(col => col.name === 'booking_status');
    
    if (!hasBookingStatus) {
      console.log('Adding booking_status column...');
      db.prepare("ALTER TABLE leads ADD COLUMN booking_status TEXT DEFAULT NULL").run();
      console.log('Column added successfully.');
    } else {
      console.log('booking_status column already exists.');
    }
